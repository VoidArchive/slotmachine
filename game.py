...
class Game:
    def __init__(self) -> None:
        ...
        grid_actual_height = self.grid_rows * (self.cell_size + self.padding) - self.padding
    ...
    def render_paytable(self):
        ...
        title_rect = title_text.get_rect(center=(table_bg_rect.centerx, table_bg_rect.top + 30))
        ...
        for i, symbol_data in enumerate(reversed(SYMBOLS)):
            ...
            name_rect = name_surf.get_rect(midleft=(x_symbol_name, current_y + row_height // 2))
            self.screen.blit(name_surf, name_rect)
            
            value_str = f"{symbol_data.base:<2d} x2(3) x5(4)"
            value_surf = self.small_font.render(value_str, True, (180, 185, 200))
            value_rect = value_surf.get_rect(midright=(x_value_text, current_y + row_height // 2))
    ...
    def draw_grid(self):
        ...
        grid_bg = pygame.Rect(left - 20, top - 20, 
        ...
        for r, row in enumerate(self.grid):
            for c, emoji in enumerate(row):
                ...
                scaled_surf = pygame.transform.smoothscale(temp_surf, (scaled_width, desired_emoji_height))
                
                self.screen.blit(scaled_surf, scaled_surf.get_rect(center=rect.center))

    def draw_ui(self):
        button_color = (60, 180, 60) if self.btn_spin.collidepoint(pygame.mouse.get_pos()) else (50, 150, 50)
        ...
        self.screen.blit(space_text, space_text.get_rect(center=(self.btn_spin.centerx, self.btn_spin.bottom + 25)))
        
        pygame.draw.rect(self.screen, (40, 40, 40), self.check_force, border_radius=5)
        pygame.draw.rect(self.screen, (80, 80, 80), self.check_force, 1, border_radius=5)
        if self.force_win:
        ...
        self.screen.blit(force_text, (self.check_force.right + 15, self.check_force.centery - 12))
        ...
        credits_text = self.info_font.render(f"Credits: {self.credits}", True, (255, 255, 255))
        ...
        pygame.draw.rect(self.screen, (80, 120, 200), self.btn_paytable, 2, border_radius=10)
        paytable_text = self.small_font.render("PAYTABLE", True, (255, 255, 255))
        self.screen.blit(paytable_text, paytable_text.get_rect(center=self.btn_paytable.center))
        ...
        if time_elapsed < self.spin_duration:
            ...
            if payout > 0:
                ...
                for pattern_type, row, col, _ in self.win_patterns:
                    ...
                    particle_x = self.grid_draw_left + col * cell_plus_padding + self.cell_size // 2
                    particle_y = self.grid_draw_top + row * cell_plus_padding + self.cell_size // 2
                    color = (0, 255, 0) if pattern_type in ("horizontal", "vertical") else (255, 215, 0)
